        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("LogFormat", targetDepth)) {
                    context.nextToken();
                    loggingConfig.setLogFormat(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("ApplicationLogLevel", targetDepth)) {
                    context.nextToken();
                    loggingConfig.setApplicationLogLevel(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("SystemLogLevel", targetDepth)) {
                    context.nextToken();
                    loggingConfig.setSystemLogLevel(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("LogGroup", targetDepth)) {
                    context.nextToken();
                    loggingConfig.setLogGroup(context.getUnmarshaller(String.class).unmarshall(context));
                }
